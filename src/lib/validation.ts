...
export const phoneSchema = z
  .string()
  .regex(/^09\d{8}$/, 'Debe ser un teléfono válido de Ecuador (10 dígitos, inicia con 09)')
  .optional()
  .or(z.literal(''))
...
export const advancedSettingsSchema = z.object({
  cancellation_policy_hours: z.number().min(0, 'Mínimo 0 horas').max(168, 'Máximo 168 horas (7 días)'),
  ...
  min_booking_hours: z.number().min(0, 'Mínimo 0 horas').max(72, 'Máximo 72 horas (3 días)'),
  max_booking_days: z.number().min(1, 'Mínimo 1 día').max(365, 'Máximo 365 días (1 año)'),
  enable_reminders: z.boolean(),
  reminder_hours_before: z.number().min(1, 'Mínimo 1 hora').max(168, 'Máximo 168 horas (7 días)'),
...
export const employeeFormSchema = z.object({
  ...
  position: z.string().min(1, 'La posición es requerida').max(100, 'Máximo 100 caracteres'), // ✅ REQUERIDO
...
export const clientFormSchema = z.object({
  ...
  {
    message: 'Debe proporcionar al menos un método de contacto (teléfono o email)',
