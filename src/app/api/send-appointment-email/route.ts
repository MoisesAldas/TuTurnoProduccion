...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!business?.name || !business?.address) {
      return NextResponse.json(
        { error: "Datos del negocio no encontrados o incompletos" },
        { status: 404 },
      );
    }
    if (services.length === 0) {
      return NextResponse.json(
        { error: "Datos del servicio no encontrados o incompletos" },
        { status: 404 },
      );
    }
    if (!employee?.first_name || !employee?.last_name) {
      return NextResponse.json(
        { error: "Datos del empleado no encontrados o incompletos" },
        { status: 404 },
      );
    }
    ...
    console.log(
      ...
      businessOwner
        ? `${businessOwner.first_name} ${businessOwner.last_name} (${businessOwner.email})`
        : "NOT FOUND",
    );
    ...
    console.log(
      "📧 Sending appointment confirmation email to CLIENT:",
      client.email,
    );
    ...
    if (businessOwner) {
      ...
      console.log(
        "📧 Sending new appointment notification to BUSINESS:",
        businessOwner.email,
      );
