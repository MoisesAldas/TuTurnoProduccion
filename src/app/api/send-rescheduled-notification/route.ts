...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      changes.oldEmployeeId &&
      changes.oldEmployeeId !== appointment.employee_id
    ) {
    ...
    const dateChanged =
      changes.oldDate && changes.oldDate !== appointment.appointment_date;
    const timeChanged =
      changes.oldTime && changes.oldTime !== appointment.start_time;
    const employeeChanged = oldEmployee !== null;

    // Preparar datos para el email
    // NUEVO: Generar token de seguridad para los botones de acción
    const token = generateAppointmentToken(appointmentId);

    const emailData = {
      to: client.email,
      userName: `${client.first_name} ${client.last_name}`,
      appointmentId, // NUEVO: Agregar ID para los botones
      token, // NUEVO: Agregar token para los botones
      data: {
        ...
        oldAppointmentDate,
        oldAppointmentTime:
          changes.oldTime?.substring(0, 5) ||
          appointment.start_time.substring(0, 5),
        oldAppointmentEndTime:
          changes.oldEndTime?.substring(0, 5) ||
      ...
      const businessEmailData = {
        ...
        data: {
          ...
          oldEmployeeName: oldEmployee
            ? `${oldEmployee.first_name} ${oldEmployee.last_name}`
            : `${newEmployee.first_name} ${newEmployee.last_name}`,
          oldEmployeePosition: oldEmployee
