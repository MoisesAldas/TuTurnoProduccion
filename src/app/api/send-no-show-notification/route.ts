...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generar política de cancelación
    const cancellationPolicy = business.cancellation_policy_hours
      ? `Por favor, recuerda que es importante cancelar tus citas con al menos ${business.cancellation_policy_hours} horas de anticipación si no podrás asistir. Esto permite que otros clientes puedan aprovechar ese horario.`
      : 'Por favor, recuerda que es importante cancelar tus citas con anticipación si no podrás asistir. Esto permite que otros clientes puedan aprovechar ese horario.'
    ...
    console.log('📧 Sending no-show email to:', client.email)

    // Enviar email
    const emailResponse = await fetch(
      `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/send-no-show-email`,
      {
        method: 'POST',
        headers: {
    ...
    if (emailResponse.ok) {
      ...
      return NextResponse.json({
        success: true,
        message: 'Email de no-show enviado',
        data: result
      }, { status: 200 })
    } else {
      const errorText = await emailResponse.text()
      console.error('⚠️ Failed to send no-show email:', errorText)
    ...
    console.error('💥 Error in send-no-show-notification:', error)
    return NextResponse.json({
