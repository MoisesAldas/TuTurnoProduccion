...
export async function middleware(req: NextRequest) {
  ...
  const isBusinessProfile = req.nextUrl.pathname.match(
    /^\/business\/[a-zA-Z0-9-]+$/
  );
  // Detectar si es la ruta de reserva (/business/[id]/book)
  const isBusinessBooking = req.nextUrl.pathname.match(
    /^\/business\/[a-zA-Z0-9-]+\/book/
  );
  ...
  const isBusinessDashboard = req.nextUrl.pathname.startsWith(
    "/dashboard/business"
  );
